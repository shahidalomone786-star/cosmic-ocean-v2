...
const ItemCard = memo(function ItemCard({ item, onRemove, onNoteChange, onOpenDocument }: { item: SavedItem; onRemove: (id: string) => void; onNoteChange: (id: string, note: string) => void; onOpenDocument: (item: SavedItem) => void }) {
  ...
  return <article data-testid={`card-saved-item-${item.id}`} className="group rounded-xl border border-white/[0.07] bg-white/[0.025] p-3.5 transition-colors hover:border-violet-300/20">
    ...
    {editingNote && <div className="mt-2 flex gap-1.5"><input data-testid={`input-note-${item.id}`} value={note} onChange={event => setNote(event.target.value)} placeholder="A note for later" className="min-w-0 flex-1 rounded-lg border border-white/[0.08] bg-white/[0.04] px-2.5 py-1.5 text-[11px] text-white/80 outline-none focus:border-violet-300/40" /><button type="button" data-testid={`button-save-note-${item.id}`} onClick={() => { onNoteChange(item.id, note); setEditingNote(false); }} className={buttonClass}>Save</button></div>}
