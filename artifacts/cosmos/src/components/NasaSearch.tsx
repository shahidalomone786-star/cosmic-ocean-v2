  ...
  return (
    ...
    >
      <motion.div
        ...
        className={`relative w-full max-w-2xl max-h-[90vh] flex flex-col rounded-[2rem] overflow-hidden transform-gpu ${
          lm
            ? 'bg-white/97 border border-gray-200 shadow-[0_40px_80px_-16px_rgba(0,0,0,0.25)]'
            : 'border border-white/[0.09] bg-[rgba(7,7,12,0.92)] backdrop-blur-xl shadow-[0_40px_80px_-16px_rgba(0,0,0,0.95),inset_0_1px_0_rgba(255,255,255,0.08)]'
        }`}
      ...
      >
        ...
        <button
          onClick={onClose}
          className={`absolute top-4 right-4 z-10 w-8 h-8 flex items-center justify-center rounded-full border backdrop-blur-sm transition-colors duration-200 text-lg leading-none ${
            lm
              ? 'border-gray-200 bg-white text-gray-500 hover:text-gray-900 hover:bg-gray-100'
              : 'border-white/15 bg-black/50 text-white/60 hover:text-white hover:bg-white/10'
          }`}
        >
          ×
        </button>
                  ...
                  >
                    {av.image ? (
                      <img
                        ...
                        className={`w-11 h-11 rounded-full object-cover border group-hover:scale-105 transition-all duration-200 shadow-lg ${lm ? 'border-gray-200 group-hover:border-gray-400' : 'border-white/15 group-hover:border-white/50'}`}
                      />
                    ) : (
                      <div className={`w-11 h-11 rounded-full flex items-center justify-center text-sm group-hover:scale-105 transition-all duration-200 ${lm ? 'bg-gray-100 border border-gray-200 text-gray-500 group-hover:border-gray-400' : 'bg-white/10 border border-white/15 text-white/60 group-hover:border-white/40'}`}>
                        {av.name.charAt(0)}
                      </div>
                    )}
                    <span className={`text-[9px] transition-colors duration-200 w-12 text-center truncate leading-tight ${lm ? 'text-gray-400 group-hover:text-gray-700' : 'text-white/40 group-hover:text-white/70'}`}>
