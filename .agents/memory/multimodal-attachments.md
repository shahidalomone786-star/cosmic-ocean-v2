...
**How to apply:** Keep document ingestion separate, enforce five images/10 MB input limits, resize browser output to at most 768px with quality 0.7, validate signatures server-side, preserve image metadata in message history, route image turns to Groq `qwen/qwen3.6-27b` with only system plus current user content and a 1500-token completion ceiling, keep text turns on `openai/gpt-oss-120b` with history and 4000 tokens, send every Groq request through the shared five-key rotation helper with 413/429 failover, and sanitize final-answer output without removing the reasoning channel.