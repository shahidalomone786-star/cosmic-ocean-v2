...
**How to apply:** Keep document ingestion separate, enforce five images/10 MB input limits, validate signatures server-side, preserve image metadata in message history, route image turns to Groq `qwen/qwen3.6-27b`, and keep text turns on `openai/gpt-oss-120b` through the same Groq key pool.