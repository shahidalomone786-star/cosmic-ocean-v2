...
**How to apply:** Keep document ingestion separate, enforce five images/10 MB input limits, validate signatures server-side, preserve image metadata in message history, route image turns to Groq `llama-3.2-11b-vision-preview`, keep text turns on `openai/gpt-oss-120b`, and show the exact text-only capability notice when vision is unavailable.