...
Cosmos keeps synthesis server-side through `@andresaya/edge-tts`, using the supported `en-US-AvaMultilingualNeural` voice, bounded frontend chunking, an abortable ordered audio queue with one-phrase-ahead prefetch, and a bounded message-level background prefetch cache. Microsoft’s live catalog does not provide `en-US-JennyMultilingualNeural`.
